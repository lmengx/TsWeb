/**
 * 服务器激活/停用统一入口：注册 REST 实例 + 建立/释放 SSE 常连 + 注册 webhook 推流。
 * 所有「添加 / 编辑 / 轮换密钥 / 删除」服务器的接口必须走这里，避免遗漏任一链路。
 *
 * 背景：日志推送主通道是 后端→插件的 SSE 常连（无条件建立）；webhook 是附加通道（依赖 logWebhook 开关）。
...
import { reRegisterWebhook, updatePluginWebhook } from './webhookRegistration.js'

/**
 * 激活（添加 / 更新 / 轮换密钥后调用）：
 *  - 注册/更新 tshockService REST 实例（registerServer 内部会重启心跳探活）
 *  - 建立后端 → 插件的 SSE 常驻长连接（connect 内部先断开旧连接；含指数退避自动重连）
 *  - 注册 webhook 推流（reRegisterWebhook 内部判断 logWebhook.enabled，启用才注册）
...
export function activateServer(server) {
  ...
  if (server.enabled) {
    connect(server)
    reRegisterWebhook(server.id).catch(() => {})
  } else {
...
/**
 * 停用（删除服务器时调用）：注销 REST 实例 + 释放 SSE 常连 + 注销插件端 webhook。
 * @param {string} id 服务器 id
 */
export function deactivateServer(id) {
  unregisterServer(id)
  disconnect(id)
  updatePluginWebhook(id, null).catch(() => {})
}
