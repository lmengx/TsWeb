...
public class Challenger : TerrariaPlugin
{
    ...
    public void MiningArmor(Player player, Config config)
    {
        ...
        var armor = player.armor;
        if ((armor[0].type == 88 || armor[0].type == 4008) && armor[1].type == 410 && armor[2].type == 411)
        {
            foreach (var effect in any)
            {
                TShock.Players[player.whoAmI].SetBuff(effect, 180, false);
    ...
    public void OnTileEdit(object o, TileEditEventArgs args)
    {
        if (Main.tile[args.X, args.Y] is { } tile && config.Tile.Contains(tile.type) && args.Action == EditAction.KillTile && args.EditData == 0)
        {
            ...
            if (plr != null)
            {
                var tPlayer = plr.TPlayer;
                var armor = tPlayer.armor;
                bool hasMiningArmor = (armor[0].type == 88 || armor[0].type == 4008)
                                      && armor[1].type == 410
                                      && armor[2].type == 411;
                if (hasMiningArmor && config.MiningArmor_1)
                {
                    args.Handled = true;
    ...
    void Mine(TSPlayer plr, int x, int y, int type)
    {
        ...
        if (plr.IsSpaceEnough(item.type, count))
        {
            ...
            plr.SendMessage(string.Format("[c/95CFA6:<挑战者:挖矿套>] 连锁挖掘了 [c/95CFA6: {0} {1}].", count, item.type == 0 ? "未知" : item.Name), Color.White);
        }
        else
        {
            plr.SendInfoMessage(string.Format("[c/95CFA6:<挑战者:挖矿套>] 背包已满，还需空位：[c/95CFA6:{0}] 以放入 [c/95CFA6:{1}] .", count, item.Name));
    ...
    public void ChlorophyteArmorEffect(Player player)
    {
        ...
        var cplayer = Collect.cplayers[player.whoAmI];
        if (flag)
        {
            if (!cplayer.ChlorophyteArmorEffectLife)
            {
                // 首次穿上：记录基础值 + 加奖励
                cplayer.ChlorophyteArmorBaseLife = player.statLifeMax;
                player.statLifeMax += Any;
                cplayer.ExtraLife += Any;
                NetMessage.SendData(16, -1, -1, NetworkText.Empty, player.whoAmI, 0f, 0f, 0f, 0, 0, 0);
                cplayer.ChlorophyteArmorEffectLife = true;
                if (config.EnableConsumptionMode)
                {
                    SendPlayerText(string.Format("生命值上限 + {0}", Any), new Color(0, 255, 255), player.Center);
                }
            }
            else
            {
                // 已穿着：检查 statLifeMax 是否被 SSC 重置（如死亡后重生）
                var expected = cplayer.ChlorophyteArmorBaseLife + Any;
                if (player.statLifeMax != expected)
                {
                    player.statLifeMax = expected;
                    NetMessage.SendData(16, -1, -1, NetworkText.Empty, player.whoAmI, 0f, 0f, 0f, 0, 0, 0);
                }
            }
        }
        if (!flag && cplayer.ChlorophyteArmorEffectLife)
        {
            player.statLifeMax -= Any;
            cplayer.ExtraLife -= Any;
            NetMessage.SendData(16, -1, -1, NetworkText.Empty, player.whoAmI, 0f, 0f, 0f, 0, 0, 0);
            cplayer.ChlorophyteArmorEffectLife = false;
            cplayer.ChlorophyteArmorBaseLife = 0;
            if (config.EnableConsumptionMode)
            {
                SendPlayerText(string.Format("生命值上限 - {0}", Any), new Color(255, 0, 156), player.Center);
            }
        }
    }
    ...
    public void TikiArmorEffect(Player? pl, ProjectileAiUpdateEventArgs? args, int mode = 0)
    {
        ...
        if (flag2 && !Collect.cplayers[pl.whoAmI].TikiArmorEffectLife)
        {
            ...
            if (config.EnableConsumptionMode)
            {
                Challenger.SendPlayerText(string.Format("生命值上限 + {0}", Any), new Color(0, 255, 255), pl.Center);
            }
        }
        if (!flag2 && Collect.cplayers[pl.whoAmI].TikiArmorEffectLife)
        {
            ...
            if (config.EnableConsumptionMode)
            {
                SendPlayerText(string.Format("生命值上限 - {0}", Any), new Color(255, 0, 156), pl.Center);
            }
        }
    }
    ...
    public void BeetleArmorEffect(Player? player, PlayerDamageEventArgs? e, NpcStrikeEventArgs? args)
    {
        ...
        if (player != null)
        {
            ...
            if (flag && !Collect.cplayers[player.whoAmI].BeetleArmorEffectLife)
            {
                ...
                if (config.EnableConsumptionMode)
                {
                    SendPlayerText(string.Format("生命值上限 + {0}", any1), new Color(0, 255, 255), player.Center);
                }
            }
            if (!flag && Collect.cplayers[player.whoAmI].BeetleArmorEffectLife)
            {
                ...
                if (config.EnableConsumptionMode)
                {
                    SendPlayerText(string.Format("生命值上限 - {0}", any1), new Color(255, 0, 156), player.Center);
                }
            }
            return;
        }
    ...
    public void SpectreArmorEffect(Player player)
    {
        ...
        if (flag && clear2 == !Collect.cplayers[player.whoAmI].SpectreArmorEffectMana)
        {
            ...
            if (config.EnableConsumptionMode)
            {
                SendPlayerText(string.Format("魔力值上限 + {0}", any), new Color(0, 255, 255), player.Center + new Vector2(0f, 32f));
            }
        ...
        if (!flag && Collect.cplayers[player.whoAmI].SpectreArmorEffectMana)
        {
            ...
            if (config.EnableConsumptionMode)
            {
                SendPlayerText(string.Format("魔力值上限 - {0}", any), new Color(255, 0, 156), player.Center + new Vector2(0f, 32f));
            }
            CProjectile.CKill(Collect.cplayers[player.whoAmI].SpectreArmorEffectProjIndex);
        }
    }
    ...
    public void DisplayTips(TSPlayer tsplayer, short type)
    {
        switch (type)
        {
            ...
            case 3090:
                ...
                break;
            case 3097:
                SendPlayerText(tsplayer, string.Format("【克苏鲁之盾】\n挑战模式奖励：冲刺时获得一小段无敌时间，冷却\n{0}秒", config.CthulhuShieldTime), new Color(255, 199, 199), Main.player[tsplayer.Index].Center + new Vector2(0f, 0f));
                break;
            case 977:
                SendPlayerText(tsplayer, string.Format("【分趾袜】\n挑战模式奖励：冲刺时获得一小段无敌时间，冷却\n{0}秒", config.CthulhuShieldTime), new Color(255, 199, 199), Main.player[tsplayer.Index].Center + new Vector2(0f, 0f));
                break;
            case 984:
                SendPlayerText(tsplayer, string.Format("【忍者大师装备】\n挑战模式奖励：冲刺时获得一小段无敌时间，冷却\n{0}秒", config.CthulhuShieldTime), new Color(255, 199, 199), Main.player[tsplayer.Index].Center + new Vector2(0f, 0f));
                break;
            case 3223:
    ...
    private void EnableModel(CommandArgs args)
    {
        ...
        if (config.enableChallenge)
        {
            ...
            TSPlayer.All.SendMessage(string.Format("挑战模式已取消，您觉得太难了？[操作来自：{0}]", args.Player.Name), new Color(82, 155, 119));
            return;
        }
        ...
        TSPlayer.All.SendMessage(string.Format("挑战模式启用，祝您愉快。[操作来自：{0}]", args.Player.Name), new Color(255, 82, 165));
    }
    ...
    private void Function(CommandArgs args)
    {
                ...
                switch (armor[i].type)
                {
                    ...
                    case 5113:
                    {
                        ...
                        switch (num)
                        {
                            ...
                            case 11:
                                Main.StartRain();
                                TSPlayer.All.SendMessage(string.Format("{0} 收听了 {1} 天气预报收音广播：即将下雨", args.Player.Name, Main.worldName), new Color(167, 218, 251));
                                TSPlayer.All.SendData((PacketTypes) 7, "", 0, 0f, 0f, 0f, 0);
                                break;
                            case 1:
                            case 12:
                                Main.StopRain();
                                TSPlayer.All.SendMessage(string.Format("{0} 收听了 {1} 天气预报收音广播：不会下雨", args.Player.Name, Main.worldName), new Color(167, 218, 251));
                                TSPlayer.All.SendData((PacketTypes) 7, "", 0, 0f, 0f, 0f, 0);
                                break;
                            case 2:
                            case 13:
                                ...
                                TSPlayer.All.SendMessage(string.Format("{0} 收听了 {1} 天气预报收音广播：不会有风", args.Player.Name, Main.worldName), new Color(167, 218, 251));
                                TSPlayer.All.SendData((PacketTypes) 7, "", 0, 0f, 0f, 0f, 0);
                                break;
                            case 3:
                            case 14:
        ...
        {
            args.Player.SendInfoMessage($"状态异常，使用失败: {ex}");
