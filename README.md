...
| 功能 | 插件模块 | 后端/前端 | 说明 |
|------|----------|-----------|------|
| **白名单** | `RegisterAndAccess`（block 模式）、`UnverifiedManager` | `unverifiedRoutes` / `UnverifiedDetail.vue` | 阻止未注册玩家进入；未验证玩家列表、注册/强制登录/踢出/封禁 |
| **房屋插件** | `HouseCore` / `HouseApi`（原 plugin-son/House 并入） | `houseApi.js` / `HouseManagementView.vue` | 圈地建房、12 项细粒度权限、屋主自定义传送点、违规冻结 |
| **QQ 机器人** | `QQ.cs` / `AccountSync`（账号台账） | `botRoutes` / `qqAccountService` / `QQConfigView.vue` | QQ 绑定账号、查询玩家数据、绑定流程、多服时长聚合 |
...
| **表情指令** | `EmoteCommandManager` | `EmoteCommandView.vue` | 玩家发表情时按配置顺序执行指定指令 |
| **Boss 管理** | `BossLimit` / `BossConfigManager` / `BossProgress` | `BossLimitSettingsView.vue` / `ProgressView.vue` | Boss 进度查询、Boss 限制（召唤限制）、配置管理 |
| **物品/弹幕反作弊** | `AntiCheat` / `ItemDetection` / `ProjDetection` / `ParticleGuard` / `BugFixes` | `antiCheatRoutes` / `ItemRestrictView.vue` / `ProjRestrictView.vue` | 违禁物品/弹幕检测、全服扫描、闪电防护、反恶性 bug（登录/宝箱/召唤物限制） |
| **风控宵禁** | `RiskControl` / `Curfew` | `RiskControlSettingsView.vue` / `CurfewSettingsView.vue` | 实时风控（进服/发言/命令拦截、一键踢出）、宵禁（条目化排期 + 豁免组） |
| 注册管理 | `AutoRegister` | `RegisterLoginSettingsView.vue` | 注册模式 default / auto / disable / block，动态切换 |
...
| 世界修改器 | `WorldModify` | `worldModifyRoutes` / `worldModifyApi.js` | REST 版简易世界修改（查看状态/应用修改） |
...
| 投票系统 | — | `voteRoutes` / `VoteView.vue` | 服务器投票功能 |
...
| **QTRHacker** | Terraria 客户端注入工具（含 QHackCLR C++ 组件） |
| **整合** | 客户端整合包产物（TerrariaHelper.dll、Hero.dll、Hero_start.bat 等） |
