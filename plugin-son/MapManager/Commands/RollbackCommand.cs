...
public class RollbackCommand : HCommand
{
    ...
    public override void Execute()
    {
        ...
        var rollbackTime = (int) (DateTime.UtcNow - MapManager.Date).TotalSeconds - this.time;
        ...
        using (var reader =
            MapManager.Database.QueryReader("SELECT * FROM History WHERE Account = @0 AND Time >= @1 AND " + XYReq + " AND WorldID = @2",
        ...
        if (!this.reenact)
        {
            MapManager.Database.Query("DELETE FROM History WHERE Account = @0 AND Time >= @1 AND " + XYReq + " AND WorldID = @2",
                this.account, rollbackTime, Main.worldID);
        }
        ...
        */
        for (var i = 0; i >= 0 && i < MapManager.Actions.Count; i++)
        {
            var action = MapManager.Actions[i];
        ...
        this.sender.SendInfoMessage(this.reenact
            ? $"重现 {actions.Count} 个操作."
            : $"回溯 {actions.Count} 个操作.");
    }
}