...
[ApiVersion(2, 1)]
public class MapManager : TerrariaPlugin
{
    ...
    public MapManager(Main game) : base(game)
    {
        this.Order = 5;
    }
                ...
                if (Main.tile[X, Y].active() && tileType < 645 && !Main.tileCut[tileType] && tileType != 127)
                {
                    ...
                    switch (tileType)
                    {
                        ...
                        case 425: // Announcement
                            ...
                            if (signI >= 0 && signI < Main.sign.Length)
                            {
                                this.Queue(account, X, Y, 0, tileType, pStyle, Main.tile[X, Y].color(), text: Main.sign[signI].text);
                            }
                            else
                            {
                                this.Queue(account, X, Y, 0, tileType, pStyle, Main.tile[X, Y].color());
                            }
                            return;
                        case 124: //wooden beam, breaks sides only
                            if (X > 0 && Main.tile[X - 1, Y].active() && this.breakableSides[Main.tile[X - 1, Y].type])
                            {
                                this.LogEdit(0, Main.tile[X - 1, Y], X - 1, Y, 0, account, done);
                            }

                            if (X < Main.maxTilesX - 1 && Main.tile[X + 1, Y].active() && this.breakableSides[Main.tile[X + 1, Y].type])
                            {
                                this.LogEdit(0, Main.tile[X + 1, Y], X + 1, Y, 0, account, done);
                            }
                        ...
                        case 235: //teleporter, 3x1
                            if (X < 2 || X >= Main.maxTilesX - 2 || Y < 1) break;
                            for (var i = -1; i <= 1; i++)
                            {
                        ...
                        case 234:
                            ...
                            topY++;
                            if (topY > 0 && Main.tile[X, topY - 1].active() && this.breakableBottom[Main.tile[X, topY - 1].type])
                            {
                                this.LogEdit(0, Main.tile[X, topY - 1], X, topY - 1, 0, account, done);
                            }
                            return;
                        case 239: //bars
                            topY = Y;
                            ...
                            while (topY <= Y)
                            {
                                if (X > 0 && Main.tile[X - 1, topY].active() && this.breakableSides[Main.tile[X - 1, topY].type])
                                {
                                    this.LogEdit(0, Main.tile[X - 1, topY], X - 1, topY, 0, account, done);
                                }
                                if (X < Main.maxTilesX - 1 && Main.tile[X + 1, topY].active() && this.breakableSides[Main.tile[X + 1, topY].type])
                                {
                                    this.LogEdit(0, Main.tile[X + 1, topY], X + 1, topY, 0, account, done);
                                }
                        ...
                        case 314: //Minecart Track
                            if (X > 0 && Y > 0 && X < Main.maxTilesX - 2 && Y < Main.maxTilesY - 2)
                            {
                                for (var i = -1; i < 2; i++)
                                {
                                    for (var j = -1; j < 2; j++)
                        ...
                        case 334: //Weapon Racks
                            if (X <= 0 || Y >= Main.maxTilesY - 1) break;
                        ...
                        default:
                            ...
                            {
                                if (Y > 0 && Main.tile[X, Y - 1].active() && this.breakableBottom[Main.tile[X, Y - 1].type])
                                {
                                    this.LogEdit(0, Main.tile[X, Y - 1], X, Y - 1, 0, account, done);
                                }

                                if (Y < Main.maxTilesY - 1 && Main.tile[X, Y + 1].active() && this.breakableTop[Main.tile[X, Y + 1].type])
                                {
                                    this.LogEdit(0, Main.tile[X, Y + 1], X, Y + 1, 0, account, done);
                                }
                            ...
                            else if (Main.tileTable[tileType])
                            {
                                ...
                                for (var i = baseStart; i <= baseEnd; i++)
                                {
                                    var checkX = X + i;
                                    var checkY = Y - height;
                                    if (checkX >= 0 && checkX < Main.maxTilesX && checkY >= 0 && Main.tile[checkX, checkY].active() && this.breakableBottom[Main.tile[checkX, checkY].type])
                                    {
                                        this.LogEdit(0, Main.tile[checkX, checkY], checkX, checkY, 0, account, done);
                                    }
                                }
                            }
                            break;
    ...
    void HistoryCmd(CommandArgs e)
    {
        if (e.Parameters.Count > 0)
        {
            ...
            {
                e.Player.SendErrorMessage("用法错误! 正确用法: \n/history [账号名] [时间] [范围]");
                return;
            }
            var radius = 10000;
